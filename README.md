...
纯 AI 开发的一个面向 A 股 ETF 的中短线选股与持仓监控工具。项目会从东方财富获取全市场 ETF 列表，从腾讯财经和 mootdx 获取历史 K 线、实时行情，然后用动量、量能、技术面、趋势等多因子模型生成 ETF 排名；同时提供命令行日报、Web Dashboard、独立实时行情 API 服务。
...
- 持仓监控：保存关注/持仓 ETF，实时查看行情和卖出参考信号；每次刷新会标记模型信号/卖出信号的前后变动，并通过飞书机器人发送以 `QUANT` 开头的提醒；自动刷新仅在交易日盘中运行，收盘后和节假日自动暂停。
...
Web Dashboard 的“回测1月”默认使用 `models.active_backtest_scheme = before_close_15m`，方案名称为“收盘前15分钟”，对每只榜单 ETF 最近约 22 个交易日做单标的规则回测。普通刷新 `/api/refresh` 只更新选股和实时行情，不自动跑回测；回测由“手动回测”按钮调用 `/api/backtest/run`，或在收盘后由后台调度器自动触发一次。

当前回测方案配置位于 `config.json` 的 `models.backtest.before_close_15m`：
...
| --- | --- |
| `action` | `buy` 或 `sell` |
| `label` | 前端展示文本：买入（收盘前15分钟） / 卖出（收盘前15分钟） |
